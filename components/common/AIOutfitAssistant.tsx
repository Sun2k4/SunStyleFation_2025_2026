...
import { Send, Sparkles, User, Bot, MessageCircle, X, Minimize2, ChevronDown } from 'lucide-react';
import { Chat } from "@google/genai";
import { createStylistChat } from '../../service/geminiService';
...
const AIOutfitAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', role: 'model', text: "Hi! I'm Sunny, your personal AI stylist. ✨\n\nI can help you with:\n• Outfit coordination\n• Style advice\n• Gift ideas" }
  ]);
  ...
  useEffect(() => {
    try {
      ...
      setMessages(prev => [...prev, {
        id: 'err', role: 'model', text: "I'm currently offline (API Key missing). Please try again later."
      }]);
    }
  }, []);
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const botMessage: Message = { 
        ...
        role: 'model', 
        text: responseText || "I'm having a bit of trouble connecting to my fashion database right now. Try again?" 
      };
  ...
  const handleChipClick = (text: string) => {
    setInput(text);
    // Optional: Auto-focus input or auto-send could go here
  };
        ...
        <div className="fixed bottom-24 right-4 md:right-6 z-50 w-[calc(100vw-2rem)] md:w-[380px] h-[600px] max-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-100 flex flex-col overflow-hidden animate-fade-in-up origin-bottom-right font-sans">
           ...
           <div className="bg-gradient-to-r from-primary-500 to-primary-600 p-4 flex items-center justify-between shadow-sm flex-shrink-0">
              <div className="flex items-center gap-3">
                 ...
                 <div>
                   <h3 className="text-white font-bold text-lg leading-tight">Stylist Sunny</h3>
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border border-gray-100 ${msg.role === 'user' ? 'bg-gray-100' : 'bg-white'}`}>
                      {msg.role === 'user' ? <User size={16} className="text-gray-600" /> : <Bot size={16} className="text-primary-500" />}
                    </div>
           ...
           <div className="bg-white border-t border-gray-100 p-2">
             {/* Quick Prompts - Only show if not loading */}
